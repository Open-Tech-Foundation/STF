...
An STF **value** is exactly one of fourteen kinds:
...
These fourteen kinds are **mutually distinct**. In particular:
...
* Implementations **MUST NOT** encode `BigInt`, `Decimal`, `Date`, `Timestamp`, `Binary`,
  `Geometry`, `Time`, or `Duration` as strings carrying a marker prefix or any other in-band sentinel.
...
The eight names defined by STF are `BIGINT`, `DECIMAL`, `DATE`, `TIMESTAMP`, `BINARY`,
`Geometry` (alias `GEOMETRY`), `Time` (alias `TIME`), and `Duration` (alias `DURATION`).
For backwards compatibility the upper-case aliases `GEOMETRY`/`TIME`/`DURATION` are also
recognised; the canonical spelling for the three new primitives is `Geometry`/`Time`/`Duration`.

* Names are matched **byte-for-byte** against the canonical set. Parsers **MUST** also accept
  the upper-case aliases for `Geometry`, `Time`, and `Duration`. `Date(…)`, `date(…)`,
...
* An identifier immediately followed by `(` in value position is in the **reserved namespace**
  ...
  2. it is an ASCII case-insensitive match of one of the eight names above.

  A reserved name that is not a byte-for-byte match of one of the eight (including aliases)
  **MUST** be rejected with `ERR_UNKNOWN_CONSTRUCTOR`. Rule 1 covers `CUSTOM(…)`, `MY_TYPE(…)`, `Date(…)`, and
...
```
geometry_payload = quoted_type "," ws coordinates_json
quoted_type      = '"' ("Point" | "LineString" | "Polygon" | "MultiPoint" | "MultiLineString" | "MultiPolygon") '"'
coordinates_json = JSON array of numbers/arrays (GeoJSON-compatible nesting)
```

* `GEOMETRY(...)` is accepted as an alias for `Geometry(...)`.
...
A time of day without date or offset — distinct from `Timestamp` (§10.4).
...
Payload is a quoted string `HH:mm` with optional `:ss` and optional `.<fraction>`, or the bare form `HH:mm` (both accepted for ergonomics; canonical serialization uses the quoted form).

* Valid ranges: `hour 00-23`, `minute 00-59`, `second 00-59` when present.
* Fractional part, if present, has **1-9 digits**; trailing zeros are preserved (`.100` ≠ `.1`).
* No timezone/offset is permitted — timezone-aware values belong to `TIMESTAMP` or a future primitive.
* `GEOMETRY`/`Time` bare `09:30` outside a quoted string is also accepted by this version's parser for backwards compatibility.

Examples of **invalid**: `Time("24:00")`, `Time("12:60")`, `Time("09:30:60")`, `Time("abc")`, empty payload — all `ERR_INVALID_CONSTRUCTOR_PAYLOAD`.

### 10.8 `Duration(...)` — Elapsed Time

An amount of elapsed time, ISO-8601 compatible.
...
Payload is a quoted ISO-8601 duration string (bare `P…` also accepted; canonical serialization quotes it):

```
duration = "P" ( date_part | time_part | date_part time_part )
date_part = [ n "Y" ] [ n "M" ] [ n "W" ] [ n "D" ]
time_part = "T" [ n "H" ] [ n "M" ] [ n ["." n] "S" ]
n = 1*DIGIT
```

* At `least one` component must be present (`P` / `PT` alone rejected).
* `Y/M/W/D` must be integers, `S` may be decimal. `H`/`S` (and time `M`) may only appear after `T`; `Y/W/D` may not appear after `T`.
* Calendar-based durations `P1Y`/`P1M` are **not** reduced to seconds — they have no fixed second count.
* `Duration("invalid")`, `Duration("P")`, `Duration("P1DT")` (empty time section) are `ERR_INVALID_CONSTRUCTOR_PAYLOAD`.

**Time vs Duration vs Timestamp:**

| Kind | Meaning |
| :--- | :--- |
| `Timestamp` | *when* (instant with offset) |
| `Time` | *time of day* (no date) |
| `Duration` | *how long* (elapsed) |
...
constructor_name   = "BIGINT" | "DECIMAL" | "DATE" | "TIMESTAMP" | "BINARY"
                   | "Geometry" | "GEOMETRY" | "Time" | "TIME" | "Duration" | "DURATION" ;
payload            = { char_not_paren } ;
...
constrained by §10.2–§10.8, and the rules in §5, §8.3, and §11.2–§11.3 are context-sensitive.
